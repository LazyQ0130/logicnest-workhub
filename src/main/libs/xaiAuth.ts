...
function renderCallbackHtml(success: boolean, message: string): string {
  ...
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><title>LogicNest WorkHub · xAI Login</title>
<style>
...
function waitForCallback(expectedState: string): Promise<{ code: string }> {
  return new Promise((resolve, reject) => {
    ...
    const server = http.createServer((req, res) => {
      ...
      res.end(renderCallbackHtml(true, 'You can now close this tab and return to LogicNest WorkHub.'));
