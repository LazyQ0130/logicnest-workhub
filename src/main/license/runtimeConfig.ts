...
export const LicenseRuntimeConfigError = {
  ...
  InvalidTrustedCa: 'invalid_trusted_ca',
...
export function resolveLicenseRuntimeConfig(
  ...
  if (!options.isPackaged) {
    return {
      config: {
        ...
        trustedCaPem: normalizeOptionalString(options.env.LOGICNEST_LICENSE_CA_PEM),
      },
    };
  }
  ...
  const trustedCaPem = normalizeOptionalString(parsed.trustedCaPem);
  if (trustedCaPem && !isCertificateAuthority(trustedCaPem)) {
    return { config: {}, error: LicenseRuntimeConfigError.InvalidTrustedCa };
  }
...
function isCertificateAuthority(value: string): boolean {
  if (/PRIVATE KEY/i.test(value)) return false;
  try {
    return new X509Certificate(value.trim()).ca;
  } catch {
    return false;
  }
}
