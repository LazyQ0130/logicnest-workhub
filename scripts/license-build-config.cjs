...
function createLicenseBuildConfig(env, options = {}) {
  ...
  const trustedCaPem = resolveTrustedCaPem(env, options.repositoryRoot || process.cwd());
...
function resolveTrustedCaPem(env, repositoryRoot) {
  let certificateInput = normalizePem(env.LOGICNEST_LICENSE_CA_PEM);
  const certificateFile = typeof env.LOGICNEST_LICENSE_CA_FILE === 'string'
    ? env.LOGICNEST_LICENSE_CA_FILE.trim()
    : '';

  if (!certificateInput && certificateFile) {
    const resolvedPath = path.isAbsolute(certificateFile)
      ? certificateFile
      : path.resolve(repositoryRoot, certificateFile);
    try {
      certificateInput = fs.readFileSync(resolvedPath, 'utf8').trim();
    } catch {
      throw new Error('[LicenseBuild] LOGICNEST_LICENSE_CA_FILE could not be read.');
    }
  }
  if (!certificateInput) return undefined;
  if (/PRIVATE KEY/i.test(certificateInput)) {
