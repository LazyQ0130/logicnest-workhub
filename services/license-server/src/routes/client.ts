...
export const registerClientRoutes = async (app: FastifyInstance, options: { db: PrismaClient; config: AppConfig; service?: ClientService; catalogService?: CatalogService; desktopReleaseService?: DesktopReleaseService }) => {
  ...
  app.get('/app-updates/check', async (request) => {
    ...
    if (!release || !release.asset) return { code: 0, data: {} };
  ...
  app.get('/app-updates/releases/:id/download', async (request, reply) => {
    ...
    const asset = await desktopReleaseService.getDownload(params.id);
    reply
  ...
  app.get('/catalog', { preHandler: auth }, async (request, reply) => {
    ...
    const items = await catalogService.listPublished(query.kind);
